let globleData,globleRoom,selfObjectId,connectionData;
const socket = io("https://whatsapp-u4to.onrender.com/", { transports: ["websocket"] })
...
const searchInput = document.getElementById('seachByname')
...
async function fetchConnectins(token){
        let data = await fetch(`https://whatsapp-u4to.onrender.com/chat/getCon`,{
        method:'POST',
...
console.log(inp,frendId);
    ...
    globleRoom=inp;
    fetch(`https://whatsapp-u4to.onrender.com/chat/getMsg`,{
        method:'POST',
        headers:{'Content-type':'Application/json',"authorization":`bearer ${JSON.parse(localStorage.getItem('token'))}`,"refresh":`bearer ${JSON.parse(localStorage.getItem("refreshToken"))}`},
...
video.addEventListener("click", ()=>{
    socket.emit("chat",{room:globleRoom,msg:`<a href="https://friendly-frangollo-9ce165.netlify.app/option?room=${globleRoom}&usertype=t">join video call</a>`,sendBy:selfObjectId,time:Date(Date.now())});
    window.location.href=`https://friendly-frangollo-9ce165.netlify.app/option?room=${globleRoom}&usertype=c`
})
vice.addEventListener("click", ()=>{
    socket.emit("chat",{room:globleRoom,msg:`<a href="https://friendly-frangollo-9ce165.netlify.app/vice.html?room=${globleRoom}&usertype=t">join vice call</a>`,sendBy:selfObjectId,time:Date(Date.now())});
    window.location.href=`https://friendly-frangollo-9ce165.netlify.app/vice?room=${globleRoom}&usertype=c`
})
...
function showNameAndStatus(inp){
    fetch(`https://whatsapp-u4to.onrender.com/chat/findOne/${inp}`,{
        method:'GET',
        headers:{'Content-type':'Application/json',"authorization":`bearer ${JSON.parse(localStorage.getItem('token'))}`,"refresh":`bearer ${JSON.parse(localStorage.getItem("refreshToken"))}`},
    }).then((res)=>res.json()).then((res)=>{
        console.log(res);
...
logout.addEventListener("click",()=>{
    fetch(`https://whatsapp-u4to.onrender.com/user/logout`,{
        method:'POST',
